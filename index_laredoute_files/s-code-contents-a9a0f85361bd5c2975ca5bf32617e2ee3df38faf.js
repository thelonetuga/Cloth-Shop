var s_account=window.wa_data.reportSuite+ ",redouteprodalltheseus";
if (window.wa_data && window.wa_data.reportSuite && window.wa_data.reportSuite.indexOf("dev")!=-1){ 
  var s_account=window.wa_data.reportSuite+ ",redoutedevalltheseus"; 
}

var s_adb=s_gi(s_account);
window.s_adb = s_adb ;
console.log('******* NEW S.CODE ******');

s_adb.currencyCode = window.wa_data.currencyCode;
s_adb.trackDownloadLinks = true;
s_adb.trackExternalLinks = true;
s_adb.trackInlineStats = false;
s_adb.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls";
s_adb.linkInternalFilters = "javascript:,.laredoute.fr,.laredoute.ch,.laredoute.ru,.laredoute.be,.laredoute.co.uk,.laredoute.it,.laredoute.com,.laredoute.es,.laredoute.pt,.laredoute.pl,.cataloguesolutions.com,localhost,.ogone.com,survey-laredoute.,partenaires.transcred.com,paypal.com,secure.ogone.com";
s_adb.linkLeaveQueryString = false;
s_adb.linkTrackVars = "None";
s_adb.linkTrackEvents = "None";
s_adb.cookieLifetime=31536000; // 365 days in seconds
s_adb.charSet = "UTF-8";
s_adb.visitorNamespace = "redcats";
s_adb.trackingServer = "metrics.laredoute.fr";
s_adb.trackingServerSecure = "smetrics.laredoute.fr";
s_adb.dc = 122;

/*-------------------------------Add in a javascript container before site catalyst-------------------------------------*/

s_adb.pageLoadStatus = true;
s_adb.usePlugins = true ;
console.log("******** PAGE LOAD STATUS INIT = " + s_adb.pageLoadStatus);
function s_doPlugins(s) {
   console.log("******** PAGE LOAD STATUS PLUG IN= " + s_adb.pageLoadStatus);
...
//    s.tnt = s.trackTNT();
    ...
    //getTimeParting for collecting YEAR / MONTH / DAY / HOURS
    s.eVar65 = s.getTimeParting(1,"n");
    s.prop65 = s.eVar65 ;

//Collecte du paramètre cod ou omniturecode dans la variable campaign
    var url = s.w.location;
    url = url + "";
    if (url.indexOf('cod=') > -1) {
      	var cod = s.Util.getQueryParam('cod') ;
      if ( cod.indexOf('#') > -1 ) {
      	cod = cod.split('#')[0];
      }
        s.campaign = s.getValOnce(cod, 's_campaign', 0);
        if (s.campaign) {
            s.eVar35 = 'D=v0';
            //s.eVar24 = s.crossVisitParticipation(s.campaign, 's_ev24', '21', '10', '>', 'purchase', 0);
        }
    }
    else if (url.indexOf('omniturecode=') > -1) {
      	var omniturecode = s.Util.getQueryParam('omniturecode') ;
      if ( omniturecode.indexOf('#') > -1 ) {
      	omniturecode = omniturecode.split('#')[0];
      }
        s.campaign = s.getValOnce(omniturecode, 's_campaign', 0);
        if (s.campaign) {
            s.eVar35 = 'D=v0';
            //s.eVar24 = s.crossVisitParticipation(s.campaign, 's_ev24', '21', '10', '>', 'purchase', 0);
        }
  ...
  //Ajout Alexandre pour integration ABTasty
  if (window.ABTasty) {
      ABTasty.processOmniture(s);
  }

  if(s.Util.getQueryParam('zanpid')){s.eVar67=s.Util.getQueryParam('zanpid')}

    //if (s_adb.pageName) s_adb.eVar56="D=pageName";
    ...
    if(typeof(Avenseo) != 'undefined' && Avenseo.track(0) != null) {
        s_adb.prop40 = Avenseo.track(0);
        s_adb.eVar40 = Avenseo.track(0);
    }
    ...
    console.log("******** PAGE LOAD STATUS FINAL = " + s_adb.pageLoadStatus);   
    //Parameters: Load time event (mandatory), # Page loads event (mandatory), Page load type eVar(mandatory), Date (optionnal)
    if(s_adb.pageLoadStatus==true){
        s_adb.getTimePageSwitch("event41", "event42", "eVar8");
        s_adb.pageLoadStatus=false;
    }
  addNumericAndCounterEvent("event44" ,"event43", time_server);
    ...
    {
        if (numeric_value){
            s_adb.events=s_adb.apl(s_adb.events,numeric_event+"="+numeric_value,",",1);
            s_adb.events=s_adb.apl(s_adb.events,counter_event,",",1);
        }
    }

    if (s_adb.Util.getQueryParam('appvi')) {
        s_adb.prop11 = "app";
        s_adb.visitorID=s_adb.Util.getQueryParam('appvi');
    }
    s_adb.prop21 = s_adb.getPreviousValue(s_adb.pageName, "s_pv");

  s_adb.cookieLifetime=31536000; // 365 days in seconds
}
s_adb.doPlugins = s_doPlugins


/* Plugin: getTimeParting 4.0 */
s_adb.getTimeParting=function(z,h){var s=this,od=new Date("1/1/2000");if(od.getDay()!=6||od.getMonth()!=0)return"Data Not Available";else{var H,M,D,U,ds,de,tm,da=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],mo=["January","February","March","April","May","June","July","August","September","October","November","December"];d=new Date;z=z?z:0;z=parseFloat(z);h=h?h:"n";if(s._tpDST&&typeof s._tpDST[d.getFullYear()]!="undefined"){var dso=s._tpDST[d.getFullYear()].split(/,/);ds=new Date(dso[0]+
...
s_adb._tpDST={2015:"3/8,11/1",2016:"3/13,11/6",2017:"3/12,11/5",2018:"3/11,11/4",2019:"3/10,11/3",2020:"3/8,11/1",2021:"3/14,11/7",2022:"3/13,11/6",2023:"3/12,11/5",2024:"3/10,11/3",2025:"3/9,11/2",2026:"3/8,11/1",2027:"3/14,11/7",2028:"3/12,11/5",2029:"3/11,11/4",2030:"3/10,11/3"};

/*
 * Plugin: getValOnce_v1.11
 */
s_adb.getValOnce=new Function("v","c","e","t",""
 ...
 */
s_adb.crossVisitParticipation=new Function("v","cn","ex","ct","dl","ev","dv",""
...
*/
s_adb.split = new Function("l", "d", ""
+ "var i,x=0,a=new Array;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x"
+ "++]=l.substring(0,i);l=l.substring(i+d.length);}return a");


s_adb.p_gpv = new Function("k", "u", ""
+ "var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+ "=s.pt(q,'&','p_gvf',k)}return v");

s_adb.p_gvf = new Function("t", "k", ""
+ "if(t){var s=this,i=t.indexOf('='),p=i<0?t:t.substring(0,i),v=i<0?'T"
+ "rue':t.substring(i+1);if(p.toLowerCase()==k.toLowerCase())return s."
+ "epa(v)}return ''"); s_adb.trackTNT = function (v, p, b) {
    var s = this, n = "s_tnt", p = (p) ? p : n, v = (v) ? v : n, r = "", pm = false, b = (b) ? b : true; if (s.Util.getQueryParam)
        pm = s.Util.getQueryParam(p); if (pm)
 ...
 */
s_adb.apl=new Function("l","v","d","u",""
 ...
 */
s_adb.getTimePageSwitch=new Function("e1","e2","v1","d1",""
...
+"{if(s_adb.events&&s_adb.events.indexOf(e1)<0){s_adb.events=s_adb.apl(s_adb.events,e1+'="
+"'+nav_time+','+e2,',',1);}if(v1){s_adb[v1]=pnt[performance.navigation.t"
+"ype]}}}");


/* Plugin: getPreviousValue 1.1 (Minified) */
s_adb.getPreviousValue=function(e,t,s){var a,g,i=new Date,r="",n=1;if(t=t||"s_gpv",i.setTime(i.getTime()+18e5),s){n=0,a=s.split(","),g=this.events?this.events.split(","):"";for(var o=0,v=a.length;o<v;o++){for(var p=0,f=g.length;p<f;p++)if(a[o]==g[p]){n=1;break}if(1==n)break}}return 1==n&&(sessionStorage.getItem("pv_page")&&(r=sessionStorage.getItem("pv_page")),e?sessionStorage.setItem("pv_page",e):sessionStorage.setItem("pv_page","no value")),r};
